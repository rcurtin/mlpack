...
PROGRAM_INFO("Sparse Coding",
    "A utility to perform sparse coding with dictionary learning.  Given an "
    "input dataset, sparse coding seeks to find a small number of points "
    "(atoms) such that each point in the dataset can be well represented by a "
    "sparse linear combination of atoms; in addition, new points can be encoded"
    " using a pre-trained sparse coding model.",
